...
// 找一个当前使用者的箱子，如果没有就创建一个新的箱子
// 并且把箱子的钥匙（session_id）通过 Cookie 交给用户
session_start();

if(empty($_SESSION['Target']) || empty($_POST['num'])) {
  // 在游戏开始时生成随机数
  ...
  if($count < 10) {
    ...
    //   // $_POST['num'] --> 用户试一试的数字
    //   // $_SESSION['Target'] --> 被猜的数字
        $result = (int)$_POST['num'] - $_SESSION['Target'];
        if($result == 0){
          $message = '√';
          // 游戏重新开始 ，删除 Cookie 即可
...
//   // $_POST['num'] --> 用户试一试的数字
//   // $_SESSION['Target'] --> 被猜的数字
//   $result = (int)$_POST['num'] - (int)$_SESSION['Target'];
//   if($result ==  0) {
//     echo '猜对了';
//   }else if ($result < 0) {
//     echo '猜小了';
